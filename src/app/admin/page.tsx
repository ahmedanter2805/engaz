...
export default function AdminDashboard() {
  ...
  // Sidebar Component
  const Sidebar = () => (
    <div className="w-64 bg-slate-900 border-l border-slate-800 h-screen fixed right-0 top-0 p-5 flex flex-col">
      <h2 className="text-2xl font-bold text-amber-500 mb-10 flex items-center gap-2">
        ...
        {[
          ...
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${
              activeTab === item.id ? 'bg-amber-500 text-slate-900 font-bold' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
            }`}
          >
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans pr-64">
      <Sidebar />
            ...
            <motion.div key="dashboard" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl">
                  ...
                  <p className="text-4xl font-bold text-amber-500">{lawyers.length}</p>
                </div>
                <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl">
                    ...
                    {consultations.map(c => (
                      <tr key={c.id} className="hover:bg-slate-800/50">
                        ...
                        <td className="p-4 text-sm">
                          ...
                          <span className="text-amber-500">{c.booking_date} | {c.booking_time}</span>
                        </td>
                        <td className="p-4">
                          <select 
                            value={c.lawyer_id || ''} 
                            onChange={e => assignLawyerToConsultation(c.id, e.target.value)}
                            className="bg-slate-950 border border-slate-700 rounded p-2 text-sm w-full outline-none"
                          >
                        ...
                        <td className="p-4">
                          <select 
                            ...
                            className="bg-slate-950 border border-slate-700 rounded p-2 text-sm outline-none"
                          >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl h-fit">
                  ...
                  <form onSubmit={handleAddLawyer} className="space-y-4">
                    <input required type="text" placeholder="الاسم" value={lawyerForm.name} onChange={e => setLawyerForm({...lawyerForm, name: e.target.value})} className="w-full bg-slate-950 border border-slate-800 p-3 rounded-lg outline-none" />
                    <input required type="text" placeholder="المسمى (مثال: محامي نقض)" value={lawyerForm.title} onChange={e => setLawyerForm({...lawyerForm, title: e.target.value})} className="w-full bg-slate-950 border border-slate-800 p-3 rounded-lg outline-none" />
                    <input required type="number" placeholder="سنوات الخبرة" value={lawyerForm.experience_years} onChange={e => setLawyerForm({...lawyerForm, experience_years: e.target.value})} className="w-full bg-slate-950 border border-slate-800 p-3 rounded-lg outline-none" />
                    <textarea placeholder="نبذة عن المحامي" rows={3} value={lawyerForm.bio} onChange={e => setLawyerForm({...lawyerForm, bio: e.target.value})} className="w-full bg-slate-950 border border-slate-800 p-3 rounded-lg outline-none resize-none" />
                ...
                <div className="lg:col-span-2 grid gap-4">
                  {lawyers.map(l => (
                    <div key={l.id} className="bg-slate-900 border border-slate-800 p-4 rounded-xl flex justify-between items-center">
                      <div>
                        <h3 className="font-bold text-lg">{l.name} <span className="text-sm font-normal text-amber-500 bg-amber-500/10 px-2 py-1 rounded ml-2">{l.title}</span></h3>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl h-fit">
                  ...
                  <form onSubmit={handleAddCase} className="space-y-4">
                    <input required type="text" placeholder="رقم القضية (فريد)" value={caseForm.case_number} onChange={e => setCaseForm({...caseForm, case_number: e.target.value})} className="w-full bg-slate-950 border border-slate-800 p-3 rounded-lg outline-none" />
                    <input required type="text" placeholder="اسم العميل" value={caseForm.client_name} onChange={e => setCaseForm({...caseForm, client_name: e.target.value})} className="w-full bg-slate-950 border border-slate-800 p-3 rounded-lg outline-none" />
                    <input required type="text" placeholder="عنوان / موضوع القضية" value={caseForm.title} onChange={e => setCaseForm({...caseForm, title: e.target.value})} className="w-full bg-slate-950 border border-slate-800 p-3 rounded-lg outline-none" />
                    <select value={caseForm.status} onChange={e => setCaseForm({...caseForm, status: e.target.value})} className="w-full bg-slate-950 border border-slate-800 p-3 rounded-lg outline-none">
                      <option>شغالة</option>
                      <option>جاري التحقيق</option>
                      <option>تم الحكم</option>
                    </select>
                    <div className="border border-dashed border-slate-700 p-4 rounded-lg text-center cursor-pointer hover:bg-slate-800">
                ...
                <div className="lg:col-span-2 overflow-x-auto">
                  <table className="w-full text-right bg-slate-900 border border-slate-800 rounded-2xl">
                    ...
                    <tbody className="divide-y divide-slate-800">
                      {cases.map(c => (
                        <tr key={c.id}>
                          <td className="p-4 font-mono text-amber-500">{c.case_number}</td>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl h-fit">
                  ...
                  <form onSubmit={handleAddResource} className="space-y-4">
                    <input required type="text" placeholder="العنوان" value={resourceForm.title} onChange={e => setResourceForm({...resourceForm, title: e.target.value})} className="w-full bg-slate-950 border border-slate-800 p-3 rounded-lg outline-none" />
                    <select value={resourceForm.type} onChange={e => setResourceForm({...resourceForm, type: e.target.value})} className="w-full bg-slate-950 border border-slate-800 p-3 rounded-lg outline-none">
                      <option value="template">نموذج عقد (للطباعة)</option>
                      <option value="article">مقال قانوني</option>
                    </select>
                    <textarea placeholder="المحتوى النصي (اختياري)" rows={4} value={resourceForm.content} onChange={e => setResourceForm({...resourceForm, content: e.target.value})} className="w-full bg-slate-950 border border-slate-800 p-3 rounded-lg outline-none resize-none" />
                    <div className="border border-dashed border-slate-700 p-4 rounded-lg text-center cursor-pointer hover:bg-slate-800">
